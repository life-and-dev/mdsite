...
describe('runPrepareGithubCommand', () => {
  ...
  it('reads mdsite.yml, generates in the configured renderer path, and writes the GitHub Pages workflow', async () => {
    ...
    expect(workflow).toContain('            renderer/node_modules')
    expect(workflow).not.toContain('            renderer/.nuxt')
    expect(workflow).not.toContain('            renderer/build/site')
    expect(workflow).toContain('name: Setup mdsite renderer')
    expect(workflow).toContain('npm install --no-save @life-and-dev/mdsite')
    expect(workflow).toContain("node_modules', '@life-and-dev', 'mdsite', 'mdsite-nuxt")
