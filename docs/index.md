...
<div align="center"><img src="logo.svg" alt="mdsite"/></div>
<h1 align="center">Markdown Site</h1>
...
| `site.favicon`           | auto-detected favicon image        | Source image path relative to the content directory (any format `sharp` supports). `mdsite init` picks the first `logo`/`favicon` image (`.svg`/`.webp`/`.jpg`/`.png`/`.ico`) it finds, top-level first; otherwise empty. The renderer generates derived favicons into the renderer's `public/` dir.                                                                                    |
