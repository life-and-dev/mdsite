...
| `paths.ignore`            | `['AGENTS.md', 'CLAUDE.md']`       | Markdown ignore rule or list. Omitting it uses the defaults; any scalar or array replaces them, and `[]` disables them. `README.md` remains published.                                                                                                                                                                                                                       |
...
Use a scalar for one replacement rule or an array for several:
...
Bare names match at any depth. Relative paths, directory rules, `*`, and `**` are supported; `!` negation and malformed entries are rejected. Rules exclude published content and generated navigation, search, metadata, and direct menu/footer Markdown reads. Separate built-ins for hidden directories, `node_modules`, `dist`, and `*.draft.md` cannot be disabled through `paths.ignore`. See the [content directory and ignore rules reference](https://life-and-dev.github.io/mdsite/content).
