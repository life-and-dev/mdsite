...
| `site.favicon`            | auto-detected favicon image        | Source image path, resolved relative to the input dir then the project root (any format `sharp` supports). `mdsite init` picks the first `logo`/`favicon` image (`.svg`/`.webp`/`.jpg`/`.png`/`.ico`) it finds in the content directory, top-level first; otherwise empty. The renderer generates derived favicons into the renderer's `public/` dir.                                                     |
